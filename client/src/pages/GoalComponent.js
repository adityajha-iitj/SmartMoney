...
const TodoList = () => {
  ...
  const fetchTodos = async () => {
    ...
    try {
      const response = await axios.get(`/financialgoals/${userName}`);
  ...
  const addTodo = async () => {
    ...
    if (trimmedTodo !== "" && !todos.includes(trimmedTodo)) {
      try {
        const response = await axios.post("/financialgoals/add", {
          userName,
  ...
  const removeTodo = async (index) => {
    try {
      const response = await axios.post("/financialgoals/remove", {
        userName,
        goalNo: index,
      });
